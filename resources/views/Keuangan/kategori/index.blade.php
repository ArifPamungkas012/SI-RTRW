        ...
        @if ($kategoris->hasPages())
            <div class="table-pagination">
                ...
                <div class="pagination-nav">
                    ...
                    @else
                        <a href="{{ $kategoris->previousPageUrl() }}" class="page-btn">Sebelumnya</a>
                    @endif

                    {{-- Nomor halaman --}}
                    @php
                        $start = max($kategoris->currentPage() - 2, 1);
                        $end = min($kategoris->currentPage() + 2, $kategoris->lastPage());
                    @endphp
                    ...
                    @for ($page = $start; $page <= $end; $page++)
                        ...
                        @else
                            <a href="{{ $kategoris->url($page) }}" class="page-number">{{ $page }}</a>
                        @endif
                    @endfor

                    {{-- Tombol Berikutnya --}}
                    @if ($kategoris->hasMorePages())
                        <a href="{{ $kategoris->nextPageUrl() }}" class="page-btn">Berikutnya</a>
                    @else
