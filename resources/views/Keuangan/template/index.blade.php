            ...
            <form method="GET" action="{{ route('keuangan.iuran.template.index') }}"
                ...
                <div style="position:relative">
                    <input name="q" value="{{ request('q') }}" placeholder="Cari template..." style="padding:8px 12px 8px 32px;border-radius:10px;
                        ...
                        onfocus="this.style.borderColor='#10b981'" onblur="this.style.borderColor='rgba(148,163,184,0.7)'">
                ...
                <button type="submit"
                    style="padding:8px 14px;border-radius:10px;background:#0f172a;color:white;
        ...
        @if ($templates->hasPages())
            <div class="table-pagination">
                ...
                <div class="pagination-nav">
                    ...
                    @else
                        <a href="{{ $templates->previousPageUrl() }}" class="page-btn">Sebelumnya</a>
                    @endif

                    {{-- Nomor halaman --}}
                    @php
                        $start = max($templates->currentPage() - 2, 1);
                        $end = min($templates->currentPage() + 2, $templates->lastPage());
                    @endphp
                    ...
                    @for ($page = $start; $page <= $end; $page++)
                        ...
                        @else
                            <a href="{{ $templates->url($page) }}" class="page-number">{{ $page }}</a>
                        @endif
                    @endfor

                    {{-- Tombol Berikutnya --}}
                    @if ($templates->hasMorePages())
                        <a href="{{ $templates->nextPageUrl() }}" class="page-btn">Berikutnya</a>
                    @else
