...
@section('content')
    <div class="content">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:20px">
            ...
            <button type="button" id="openCreatePengeluaranModal"
        ...
        @if(session('error'))
            <div
                style="margin-bottom:14px;padding:10px 14px;border-radius:12px;
        ...
        @if ($pengeluaran->hasPages())
            <div class="table-pagination">
                <div class="pagination-info">
                    Menampilkan
                    <strong>{{ $pengeluaran->firstItem() }}</strong>
                    –
                    <strong>{{ $pengeluaran->lastItem() }}</strong>
                    dari
                    <strong>{{ $pengeluaran->total() }}</strong>
                    data
                </div>
                ...
                <div class="pagination-nav">
                    ...
                    @else
                        <a href="{{ $pengeluaran->previousPageUrl() }}" class="page-btn">Sebelumnya</a>
                    @endif

                    {{-- Nomor halaman --}}
                    @php
                        $start = max($pengeluaran->currentPage() - 2, 1);
                        $end = min($pengeluaran->currentPage() + 2, $pengeluaran->lastPage());
                    @endphp
                    ...
                    @for ($page = $start; $page <= $end; $page++)
                        ...
                        @else
                            <a href="{{ $pengeluaran->url($page) }}" class="page-number">{{ $page }}</a>
                        @endif
                    @endfor

                    {{-- Tombol Berikutnya --}}
                    @if ($pengeluaran->hasMorePages())
                        <a href="{{ $pengeluaran->nextPageUrl() }}" class="page-btn">Berikutnya</a>
                    @else
    ...
    <div id="createPengeluaranModal" class="hidden" style="position:fixed;inset:0;z-index:50;display:flex;align-items:center;justify-content:center;
        ...
        <div
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
                ...
                <button type="button" data-close-pengeluaran-modal
